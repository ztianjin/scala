...
trait Infer {
  ...
  private final val inferInfo = global.opt.inferDebug //@MDEBUG
    ...
    object AdjustedTypeArgs {
      ...
      object AllArgsAndUndets {
        def unapply(m: Result): Some[(List[Symbol], List[Type], List[Type], List[Symbol])] = Some(toLists {
          val (ok, nok)           = m partition (_._2.isDefined)
          val (okArgs, okTparams) = ok.unzip

          (okArgs, okTparams.flatten, m.values map (_ getOrElse NothingClass.tpe), nok.keys)
        })
      }
...
*/
    ...
    def inferExprAlternative(tree: Tree, pt: Type): Unit = tree.tpe match {
      case OverloadedType(pre, alts) => tryTwice {
        val (alts1, secondTry) = alts filter (alt => isWeaklyCompatible(pre.memberType(alt), pt)) match {
          case Nil  => (alts, true)
          case xs   => (xs, false)
        }
