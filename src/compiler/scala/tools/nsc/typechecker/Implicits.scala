...
trait Implicits {
  ...
  def inferImplicit(tree: Tree, pt: Type, reportAmbiguous: Boolean, isView: Boolean, context: Context): SearchResult = {
    printInference("infer implicit%s with pt = %s".format(
    ...
    if (traceImplicits && !tree.isEmpty && context.undetparams.nonEmpty)
      ...
      try {
        ...
        if (itree2.tpe.isError)
          ...
          if (matchesPt(itree2.tpe, pt.instantiateTypeParams(undetParams, tvars), undetParams)) {
            ...
            // does not check whether inferred arguments meet the bounds of the corresponding parameter
            // (see note in solvedTypes), must check again here:
            // TODO: I would prefer to just call typed instead of duplicating the code here, but this is
            // probably a hotspot (and you can't just call typed, need to force re-typecheck)
            //
            // XXX is it OK that this return tree was ignored?
            val itree3 = itree2 match {
              case TypeApply(fun, args)           => typedTypeApply(itree2, EXPRmode, fun, args)
              case Apply(TypeApply(fun, args), _) => typedTypeApply(itree2, EXPRmode, fun, args) // t2421c
              case t                              => t
            }
            val result = new SearchResult(itree3, subst)
