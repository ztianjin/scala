...
trait Contexts { self: Analyzer =>
  ...
  class Context private[typechecker] {
    ...
    // (after `this`, which contributes `newImplicits` below)
    // in most cases, it is simply the outer context
    // if we're owned by a constructor, the actual current context and the conceptual context
    // are different when it comes to scoping:
    ...
    private def nextOuter = {
      // drop the constructor body blocks (they come in varying numbers depending on
      // whether we are in the ctor call in the first statement or after)
      // scopingCtx == the constructor definition (if we were after the ctor call)
      // or the class that contains this constructor (if we are in the ctor call)
      val scopingCtx =
